            ...
            for product in flagged:
                ...
                if ai:
                    ...
                    if desc:
                        try:
                            prompt = (
                                "You are a senior Ayurvedic copywriter for Falcon Herbs (falconherbs.com).\n"
                                "Rewrite the product description for '{}'.\n\n"
                                "WRITING REQUIREMENTS — be specific, not generic:\n"
                                "- Open with a vivid sensory hook: herb's aroma, appearance, texture, or taste\n"
                                "- Include Sanskrit/Hindi name and its literal meaning if notable\n"
                                "- Describe Ayurvedic context: which dosha it balances, Rasa (taste), Virya (potency)\n"
                                "- Name 3-5 SPECIFIC traditional uses — not 'overall wellness' but precise functional language\n"
                                "  Examples: 'calming Vata-aggravated restlessness', 'nourishing Kapha-dominant hair texture',\n"
                                "  'traditionally valued for supporting digestive Agni'\n"
                                "- Include HOW TO USE: preparation method (decoction/powder/infusion), suggested quantity, timing\n"
                                "- Mention region of origin or growing conditions if it adds authenticity\n"
                                "- Tone: warm, knowledgeable, premium — like a trusted Vaidya, not a generic e-commerce listing\n\n"
                                "COMPLIANCE RULES — strictly follow:\n"
                                "- NEVER write: 'cures', 'treats', 'treatment for', 'prevents disease',\n"
                                "  'heals disease', 'clinically proven', 'medically proven', 'guaranteed to cure', 'miracle cure'\n"
                                "- USE INSTEAD: 'traditionally used for', 'Ayurvedic tradition values this for',\n"
                                "  'may support', 'supports healthy [function]', 'traditionally valued for'\n"
                                "- End with disclaimer: 'Not intended to diagnose, treat, cure, or prevent any disease.'\n\n"
                                "OUTPUT FORMAT:\n"
                                "1. Return ONLY the rewritten HTML description — no intro, no 'Certainly!', no meta-commentary\n"
                                "2. List SEO keywords at the VERY END after a '---' separator\n\n"
