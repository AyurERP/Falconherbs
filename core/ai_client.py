...
def call_ai(
    ...
    if provider == "nvidia" and OPENROUTER_API_KEY:
        try:
            or_fallback = "meta-llama/llama-3.3-70b-instruct:free"
            log.info("Cross-provider retry on OpenRouter with model=%s", or_fallback)
            result = _call_openrouter(or_fallback, messages, timeout, max_tokens)
            log.info("OpenRouter cross-provider fallback OK  |  chars=%d", len(result))
            return result
        except Exception as exc2:
            log.warning("OpenRouter cross-provider fallback also failed: %s", exc2)
