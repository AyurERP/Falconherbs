...
cursor = conn.cursor()
cursor.execute("SELECT id, action, details FROM action_log ORDER BY id DESC LIMIT 50")
rows = cursor.fetchall()
output = []
for row in rows:
    output.append({"id": row[0], "action": row[1], "details": json.loads(row[2])})
sys.stdout.buffer.write(json.dumps(output[::-1], ensure_ascii=False).encode('utf-8'))
"""
...
with open("tmp/full_db_out.json", "wb") as f:
    f.write(res.stdout)
print("Saved to tmp/full_db_out.json")
